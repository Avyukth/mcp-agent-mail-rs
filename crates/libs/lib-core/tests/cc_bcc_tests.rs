...
async fn setup_cc_bcc_messaging(tc: &TestContext) -> (i64, i64, i64, i64, i64) {
    ...
    let to_recipient_id = AgentBmc::create(&tc.ctx, &tc.mm, to_recipient_c).await.unwrap();
    ...
    let cc_recipient_id = AgentBmc::create(&tc.ctx, &tc.mm, cc_recipient_c).await.unwrap();
    ...
    let bcc_recipient_id = AgentBmc::create(&tc.ctx, &tc.mm, bcc_recipient_c).await.unwrap();

    (project.id, sender_id, to_recipient_id, cc_recipient_id, bcc_recipient_id)
}
...
async fn test_send_message_with_cc() {
    ...
    let to_entry = recipient_types.iter().find(|(id, _)| *id == to_recipient_id);
    assert!(to_entry.is_some(), "TO recipient should be in database");
    assert_eq!(to_entry.unwrap().1, "to", "First recipient should have type 'to'");

    // Find the CC recipient
    let cc_entry = recipient_types.iter().find(|(id, _)| *id == cc_recipient_id);
    assert!(cc_entry.is_some(), "CC recipient should be in database");
    assert_eq!(cc_entry.unwrap().1, "cc", "Second recipient should have type 'cc'");
}
...
async fn test_send_message_with_bcc() {
    ...
    let bcc_entry = recipient_types.iter().find(|(id, _)| *id == bcc_recipient_id);
    assert!(bcc_entry.is_some(), "BCC recipient should be in database");
    assert_eq!(bcc_entry.unwrap().1, "bcc", "Second recipient should have type 'bcc'");
}
...
async fn test_send_message_with_cc_and_bcc() {
    ...
    let to_entry = recipient_types.iter().find(|(id, _)| *id == to_recipient_id);
    assert!(to_entry.is_some(), "TO recipient should be in database");
    assert_eq!(to_entry.unwrap().1, "to", "TO recipient should have type 'to'");

    // Verify CC recipient
    let cc_entry = recipient_types.iter().find(|(id, _)| *id == cc_recipient_id);
    assert!(cc_entry.is_some(), "CC recipient should be in database");
    assert_eq!(cc_entry.unwrap().1, "cc", "CC recipient should have type 'cc'");

    // Verify BCC recipient
    let bcc_entry = recipient_types.iter().find(|(id, _)| *id == bcc_recipient_id);
    assert!(bcc_entry.is_some(), "BCC recipient should be in database");
    assert_eq!(bcc_entry.unwrap().1, "bcc", "BCC recipient should have type 'bcc'");
}
...
async fn test_cc_recipient_can_see_message() {
    ...
    let to_inbox = MessageBmc::list_inbox_for_agent(&tc.ctx, &tc.mm, project_id, to_recipient_id, 10)
        .await
        .expect("Failed to get TO recipient inbox");
    assert_eq!(to_inbox.len(), 1, "TO recipient should have 1 message");
    assert_eq!(to_inbox[0].id, msg_id, "TO recipient should see the message");

    // Check CC recipient's inbox
    let cc_inbox = MessageBmc::list_inbox_for_agent(&tc.ctx, &tc.mm, project_id, cc_recipient_id, 10)
        .await
        .expect("Failed to get CC recipient inbox");
    assert_eq!(cc_inbox.len(), 1, "CC recipient should have 1 message");
    assert_eq!(cc_inbox[0].id, msg_id, "CC recipient should see the message");
}
...
async fn test_bcc_recipient_can_see_message() {
    ...
    let bcc_inbox = MessageBmc::list_inbox_for_agent(&tc.ctx, &tc.mm, project_id, bcc_recipient_id, 10)
        .await
        .expect("Failed to get BCC recipient inbox");
    assert_eq!(bcc_inbox.len(), 1, "BCC recipient should have 1 message");
    assert_eq!(bcc_inbox[0].id, msg_id, "BCC recipient should see the message");

    // Verify BCC recipient is stored with correct type (would not be visible in a real UI)
    let recipient_types = get_recipient_types(&tc, msg_id).await;
    let bcc_entry = recipient_types.iter().find(|(id, _)| *id == bcc_recipient_id);
    assert!(bcc_entry.is_some(), "BCC recipient should exist in database");
    assert_eq!(bcc_entry.unwrap().1, "bcc", "Recipient should be marked as BCC");
}
...
async fn test_multiple_cc_recipients() {
    ...
    let cc_recipient2_id = AgentBmc::create(&tc.ctx, &tc.mm, cc_recipient2_c).await.unwrap();
    ...
    // Count CC recipients
    let cc_count = recipient_types.iter().filter(|(_, typ)| typ == "cc").count();
    assert_eq!(cc_count, 2, "Should have 2 CC recipients");

    // Both CC recipients should see the message
    let cc1_inbox = MessageBmc::list_inbox_for_agent(&tc.ctx, &tc.mm, project_id, cc_recipient_id, 10)
        .await.unwrap();
    assert_eq!(cc1_inbox.len(), 1, "First CC recipient should have 1 message");

    let cc2_inbox = MessageBmc::list_inbox_for_agent(&tc.ctx, &tc.mm, project_id, cc_recipient2_id, 10)
        .await.unwrap();
    assert_eq!(cc2_inbox.len(), 1, "Second CC recipient should have 1 message");
}
...
async fn test_multiple_bcc_recipients() {
    ...
    let bcc_recipient2_id = AgentBmc::create(&tc.ctx, &tc.mm, bcc_recipient2_c).await.unwrap();
    ...
    let bcc_count = recipient_types.iter().filter(|(_, typ)| typ == "bcc").count();
    assert_eq!(bcc_count, 2, "Should have 2 BCC recipients");

    // Both BCC recipients should see the message
    let bcc1_inbox = MessageBmc::list_inbox_for_agent(&tc.ctx, &tc.mm, project_id, bcc_recipient_id, 10)
        .await.unwrap();
    assert_eq!(bcc1_inbox.len(), 1, "First BCC recipient should have 1 message");

    let bcc2_inbox = MessageBmc::list_inbox_for_agent(&tc.ctx, &tc.mm, project_id, bcc_recipient2_id, 10)
        .await.unwrap();
    assert_eq!(bcc2_inbox.len(), 1, "Second BCC recipient should have 1 message");
}
...
async fn test_cc_recipient_mark_as_read() {
    ...
    assert!(result.is_ok(), "CC recipient should be able to mark message as read");
}
...
async fn test_bcc_recipient_acknowledge() {
    ...
    assert!(result.is_ok(), "BCC recipient should be able to acknowledge message");
}

	...
	<BlurFade delay={0}>
		<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
			<div>
				...
				<p class="text-sm md:text-base text-muted-foreground">Manage your agent mail projects</p>
			</div>
	...
	{:else if projects.length === 0}
		...
		<BlurFade delay={100}>
			<Card.Root class="p-8 md:p-12 text-center">
				<div class="mb-4 flex justify-center"><FolderKanban class="h-12 w-12 text-muted-foreground" /></div>
		...
		<BlurFade delay={100}>
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				{#each projects as project, index}
					...
					>
						<Card.Root class="h-full hover:shadow-lg hover:border-primary/50 transition-all duration-200 hover:-translate-y-1">
							<Card.Content class="p-5 md:p-6">
								<div class="flex items-start justify-between mb-4">
									...
									<ArrowRight class="h-5 w-5 text-muted-foreground group-hover:text-primary transition-colors opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all" />
								</div>
